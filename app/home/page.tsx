...
function formatCpu(value: number) {
  return value >= 1000 ? `${value / 1000} vCPU` : `${value} mCPU`;
}
...
export default async function HomePage() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-white text-stone-950 dark:bg-black dark:text-white">
      ...
      <section className="border-b border-stone-200 dark:border-stone-800">
        <div className="mx-auto max-w-7xl px-5 pb-20 pt-20 sm:px-8 sm:pb-28 sm:pt-28 lg:pt-32">
          <div className="max-w-4xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-stone-200 bg-stone-50 px-3 py-1.5 text-xs font-semibold text-stone-600 dark:border-stone-800 dark:bg-stone-950 dark:text-stone-300"><Zap className="h-3.5 w-3.5" aria-hidden="true" /> Indonesia-first Developer Cloud</div>
          ...
          <div className="mt-16 grid gap-4 md:grid-cols-3">
            {workspaceHighlights.map((item) => { const Icon = item.icon; return <div key={item.title} className="rounded-2xl border border-stone-200 bg-stone-50/70 p-5 dark:border-stone-800 dark:bg-stone-950/70"><Icon className="h-5 w-5" aria-hidden="true" /><h2 className="mt-4 text-sm font-bold">{item.title}</h2><p className="mt-1.5 text-sm leading-6 text-stone-500 dark:text-stone-400">{item.text}</p></div>; })}
          </div>
        </div>
      </section>
      ...
      <section className="border-b border-stone-200 dark:border-stone-800">
        <div className="mx-auto max-w-7xl px-5 py-16 sm:px-8 sm:py-20">
          <div className="grid gap-12 lg:grid-cols-[.8fr_1.2fr] lg:items-start">
            ...
            <div className="grid gap-4 sm:grid-cols-2">{platformFeatures.map((item) => { const Icon = item.icon; return <div key={item.title} className="rounded-2xl border border-stone-200 p-5 dark:border-stone-800"><Icon className="h-5 w-5" aria-hidden="true" /><h3 className="mt-4 text-sm font-bold">{item.title}</h3><p className="mt-1.5 text-sm leading-6 text-stone-500 dark:text-stone-400">{item.text}</p></div>; })}</div>
          </div>
        </div>
      </section>
      ...
      <section id="pricing" className="border-b border-stone-200 dark:border-stone-800">
        <div className="mx-auto max-w-7xl px-5 py-16 sm:px-8 sm:py-20">
          <div className="max-w-2xl">
            <div className="text-xs font-semibold uppercase tracking-[0.18em] text-stone-500">Developer Cloud</div>
            <h2 className="mt-3 text-3xl font-black tracking-tight sm:text-4xl">Simple plans. Clear resources.</h2>
            <p className="mt-4 text-sm leading-6 text-stone-500 dark:text-stone-400">Product names, prices, and resource limits are read from the live cloud plan catalog in the database. Nothing on this page duplicates the pricing data.</p>
          </div>
          {plans.length ? <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{plans.map((plan) => <article key={plan.id} className="flex h-full flex-col rounded-3xl border border-stone-200 bg-white p-6 shadow-sm dark:border-stone-800 dark:bg-stone-950 sm:p-7"><div className="flex items-start justify-between gap-4"><div><div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-stone-500">Developer Cloud</div><h3 className="mt-2 text-lg font-bold">{plan.name}</h3></div><span className="rounded-full border border-stone-200 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.12em] text-stone-500 dark:border-stone-700">{plan.code}</span></div><div className="mt-6 flex items-end gap-1.5"><span className="text-4xl font-black tracking-tight">{plan.priceIdr === 0 ? "Free" : formatIdr(plan.priceIdr)}</span>{plan.priceIdr > 0 ? <span className="pb-1.5 text-xs text-stone-500">/ bulan</span> : null}</div><div className="mt-6 space-y-3 border-t border-stone-200 pt-5 text-sm dark:border-stone-800"><div className="flex items-center justify-between gap-4"><span className="text-stone-500">Projects</span><span className="font-semibold">{plan.projectLimit}</span></div><div className="flex items-center justify-between gap-4"><span className="text-stone-500">CPU</span><span className="font-semibold">{formatCpu(plan.cpuMillicores)}</span></div><div className="flex items-center justify-between gap-4"><span className="text-stone-500">RAM</span><span className="font-semibold">{formatMemory(plan.memoryMb)}</span></div><div className="flex items-center justify-between gap-4"><span className="text-stone-500">Storage</span><span className="font-semibold">{plan.storageGb} GB</span></div></div><div className="mt-auto pt-7"><Link href="/login" className="inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-stone-950 text-sm font-semibold text-white hover:bg-stone-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-stone-500 dark:bg-white dark:text-black dark:hover:bg-stone-200">Start with {plan.name} <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link></div></article>)}</div> : <div className="mt-10 rounded-2xl border border-dashed border-stone-300 p-8 text-sm text-stone-500 dark:border-stone-700">The cloud catalog is empty. Publish a plan in the database and it will appear here automatically.</div>}
        </div>
      </section>
